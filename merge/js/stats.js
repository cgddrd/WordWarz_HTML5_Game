...
function addNewDefaultAchievement(description, criteria, type) {
	...
	if (type === achieveType.WORD) {
		...
		death_achievements.push(newAchievement);
...
function setDefaultAchievements() {
	...
	addNewDefaultAchievement("Played for more than 2 minutes", 120, achieveType.TIME);
	...
	addNewDefaultAchievement("Died within 60 seconds.", 60, achieveType.DEATH);
	addNewDefaultAchievement("Died within 30 seconds.", 30, achieveType.DEATH);
	addNewDefaultAchievement("Died within 10 seconds.", 10, achieveType.DEATH);
...
function checkDeathAchievements() {
	...
	for (var i = 0, len = death_achievements.length; i < len; i++) {
		
		//If the difference in time matches any of the acheivement criteria
		if (timeMinDiff <= death_achievements[i].criteria && !(death_achievements[i].obtained)) {
		
			//Award the achievement.
			death_achievements[i].obtained = true;
			addPlayerAchievement(death_achievements[i]);
